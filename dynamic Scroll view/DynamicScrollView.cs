...
public class DynamicScrollView : MonoBehaviour
{
    ...
    private IEnumerator CreateSpaceShips(int productCount, bool isMovie)
    {
        ...
        for (int s = 0; s < sprites.Count; s++)
        {
            ...
            newSpaceShip.GetComponent<ScrollViewItem>().flag = s;
            newSpaceShip.GetComponent<ScrollViewItem>().witchScroolView = witchScroolView;
            newSpaceShip.GetComponent<ScrollViewItem>().isMovie = isMovie;
            newSpaceShip.GetComponent<ScrollViewItem>().downImage(s);
            if (s == sprites.Count - 1 ) newSpaceShip.GetComponent<ScrollViewItem>().lastItem = true;
            SpaceShipObj.Add(newSpaceShip);
